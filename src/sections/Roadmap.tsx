...
function GoalList({ title, goals }: { title: string; goals: GoalItem[] }) {
  return (
    <div className="flex flex-col gap-3 sm:gap-5">
      <h3 className="inline-flex items-center gap-2 text-xl font-black uppercase tracking-[0.2em] text-[#FF8B00] sm:gap-3 sm:text-2xl lg:text-3xl lg:tracking-[0.25em]">
        {title}
      </h3>
      <div className="space-y-2 sm:space-y-3">
        {goals.map((goal) => (
          ...
          >
            ...
            <span className="flex-1 text-[10px] font-black uppercase leading-snug tracking-[0.15em] sm:text-xs sm:tracking-[0.2em] lg:text-sm lg:tracking-[0.25em]">
              {goal.label}
            </span>
          </div>
        ))}
